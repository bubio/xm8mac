...
> **完了記録（2026-07-24）:** 実際の受入結果と承認済み未実施項目は
> [38_Phase9Windows実施記録.md](38_Phase9Windows実施記録.md)に記録し、Phase 9を完了とした。
> 以下は計画時の完全試験matrixとして、将来の任意追試にも使用する。

計画時点では、Phase 9は次をすべて満たした場合だけ完了とする方針だった。実際の完了判定では、
実機受入で重大な問題がないことを確認し、未実施項目を承認済み差異として38へ明記した。
...
未確認項目を暗黙に実施済みとは記録しない。ARM64など実機を用意できない項目はbuild成功と
runtime受入を分け、未実施または承認済み差異として完了記録へ明示する。
