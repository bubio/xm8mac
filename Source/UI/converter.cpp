...
bool Converter::Init()
{
	...
	// allocate UCS-2 to shift-jis table
	...
	// make UCS-2 table (1)
	sjis = sjis_table;
	for (high=0x8000; high<0xa000; high+=0x0100) {
		for (low=0x40; low<0x100; low++) {
			if (*sjis > 0x100) {
	...
	// make UCS-2 table (2)
	for (high=0xe000; high<0xf000; high+=0x0100) {
		for (low=0x40; low<0x100; low++) {
			if (*sjis > 0x100) {
				ucs_table[*sjis] = (Uint16)(high | low);
			}
...
//
void Converter::SjisToUtf(const char *sjis, char *utf)
{
	Uint16 high;
	Uint16 low;
	Uint16 ucs;
...
void Converter::UtfNfcToSjis(const char *utf, char *sjis)
{
	...
	for (;;) {
		...
		// from table
		if (ucs_table[ucs] != 0) {
			*sjis++ = (char)(ucs_table[ucs] >> 8);
			*sjis++ = (char)(ucs_table[ucs]);
			continue;
		}
...
const Uint16 Converter::sjis_table[0x30 * 0xc0] = {
	...
	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
	0x2227, 0x2228, 0x00ac, 0x21d2, 0x21d4, 0x2200, 0x2203, 0x0000,
	0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
