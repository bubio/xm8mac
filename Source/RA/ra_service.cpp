	...
	std::string *error)
{
	if (!IsReady() || login_.state != RaLoginState::LoggedIn ||
		game_session_.state != RaGameSessionState::Loaded) {
		if (error != nullptr) {
			*error = "RA game is required before verifying media";
		}
		return false;
	}
	if (!IsMd5Hex(hash)) {
	...
	const auto verified = verified_media_game_ids_.find(hash);
	if (verified != verified_media_game_ids_.end()) {
		media_verification_.state = verified->second == game_session_.game_id ?
			RaMediaChangeState::Succeeded : RaMediaChangeState::Failed;
		if (media_verification_.state == RaMediaChangeState::Failed) {
